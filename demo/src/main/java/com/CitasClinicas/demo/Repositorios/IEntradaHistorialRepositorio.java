...
public interface IEntradaHistorialRepositorio extends JpaRepository<EntradaHistorial, Long> {
 ...
 @Query("SELECT e FROM EntradaHistorial e JOIN e.expedientes ec WHERE ec.id = :expedienteId ORDER BY e.fecha DESC")
...
// Estadísticas
 @Query("SELECT e.tipoEntrada, COUNT(e) FROM EntradaHistorial e JOIN e.expedientes ec WHERE ec.id = :expedienteId GROUP BY e.tipoEntrada")
 List<Object[]> contarEntradasPorTipo(@Param("expedienteId") Long expedienteId);
 
 // Búsquedas avanzadas
@Query("SELECT e FROM EntradaHistorial e JOIN e.expedientes ec WHERE ec.id = :expedienteId " +
...
@Param("tipo") String tipo
 ...
 @Query("SELECT e FROM EntradaHistorial e JOIN e.expedientes ec WHERE ec.paciente.id = :pacienteId " +
"AND e.fecha >= :fecha " +
...
"e.tratamiento as tratamiento) FROM EntradaHistorial e JOIN e.expedientes ec " +
"WHERE ec.id = :expedienteId " +
"ORDER BY e.fecha DESC")
